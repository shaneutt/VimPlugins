...
"
" Todo {{{2
" s0 When opening in quickview, unfold everything so that I don't need to go to the file to open the content
" s0 Prevent the color styles from leaking to other files by putting a window check
" s0 have the possibility to go to the path that is in the clipboard 
" Code organization {{{3
...
" Comments {{{3
" - Modify the comments in the user-command section, they are outdated
" Documentation {{{3
" - Add documentation about user-defined commands
" Improvements to current features {{{3
" - Add the possibility to go to the path in the clipboard add a mapping for that
" - Add the possibility to put paths with backslashes in the [c:/temp] path location at the top and go to it, now it accepts only forward slashes
" - Add a mapping to send the current file to the clipboard using the windows clip.exe command.
" - The filter should be effective only for the current window and not for other vimexplorer window. When do a filter on the current window and then change to another window, the filter is applied to that window as well, this is not correct.
" - The <esc> command to remove selection should remember the cursor position before re-issuing the g:VeLs() command because pressing it puts the cursor to the cursor position last remembered
" - To make better the <space>D (delete folder/file) mapping, something it is long... test if the deleted item is a file with filereadable if yes then don't run the delete directory command for nothing (vice-vesa)
" - Add clone directory
...
" New features {{{3
" - Maybe add or suggest in comment the mappings for Git integration in vimExplorer I added to my vimrc (see Git section in my vimrc)
" - When doing <space><esc> r! add the current full filename in the command line?
...
" Bugs {{{2
" - 2013-09-25 09:41 The grep <space>g dosen't work on my workstation and at home
" - 2013-08-21 17:54 In the history I cannot reach a directory like this one for example [D:/users/alexandre.viau/Documents/MAM/Cases/Polsat/02209046 - Polsat_Archive_To_SGL - two different ways of ERROR handling]
" - At work: the clone function dosen't work for some reason I think because of slashes in the date format
...
" - For some reason, the marks and all vimExplorer.xml is overwritten... after long time?
" Annoyances {{{3
...
" 2.5.4
" - I modified the command by adding a silent: nmap <buffer> <space><esc> :let f = input('r! ', '') \| if f != '' \| exe 'norm G' \| exe 'silent r! ' . f \| endif<cr>
"
" Variables: Plugin {{{1
...
" The maximum lenght for the history bar the top of the buffer (Tip: You may put a very large number and cut the line yourself when its too much long using the vim 'D' command)
let s:HistoryMaxLength = 9999999
...
if has('Win32')
    ...
    let g:VeLsCmdPathU = 'c:\vimfiles\ls.exe'
    " Cygwin ls command
...
if has('Win32')
    ...
    let g:VeGrepCmdPath = 'c:\vimfiles\grep.exe'
" Linux
else
...
fu! g:BuildWindow(winType)
    ...
    " <space><esc> Execute a command and insert its output in the buffer
    nmap <buffer> <space><esc> :let f = input('r! ', '') \| if f != '' \| exe 'norm G' \| exe 'silent r! ' . f \| endif<cr>
    " <space>l or <enter> Open the selected item in preview window
    ...
    if has('Win32') == 1
        " <space>c Clone file
        nmap <silent> <buffer> <space>c :cal g:VeGetPath() \| let t = g:VeFileName . '_' . substitute(strftime('%x_%X'), ':', '-', 'g') \| let t = substitute(t, '/', '-', 'g') \| cal system('copy ' . g:VePaths.FileName2Q . ' "' . t . '"') \| cal g:VeLs() \| cal search(t)<cr>
        " <space>C Open directory in shell
...
fu! g:VeSaveToFile()
    if exists("g:VeCfg") == 1
	    ...
	    if exists("g:VePath") == 1 " I added this because I had problem when running the import of the csv files for the case views project. I don't know why it was causing problems here.
	        let g:VeCfg.LastPath.Value = g:VePath
	    endif
